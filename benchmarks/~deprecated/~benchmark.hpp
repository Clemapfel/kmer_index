...
struct benchmark_arguments
{
    ...
    void add_counters_to(benchmark::State& state, size_t k, bool use_hashtable) const
    {
...
template<seqan3::alphabet alphabet_t>
static void fm_construction(benchmark::State& state, benchmark_arguments<alphabet_t> input)
{
    std::vector<std::vector<alphabet_t>> queries;
    std::vector<alphabet_t> text;
    input.generate_queries_and_text(&queries, &text, true);
